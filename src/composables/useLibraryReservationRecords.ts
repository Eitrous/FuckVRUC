...
import type {
  ...
  LibraryReservationRecordsQueryResult,
} from '@/types/library'
...
export function useLibraryReservationRecords() {
  const loading = ref(false)
  const error = ref<string>()
  const errorCode = ref<LibraryQueryErrorCode>()
  const page = ref<LibraryReservationRecordPage>()
  const fetchedAt = ref<number>()
  let requestVersion = 0

  function clearLibraryReservationRecords() {
    requestVersion += 1
    loading.value = false
  ...
  async function queryLibraryReservationRecords(
    ...
    try {
      ...
      while (currentRequest === requestVersion) {
        const result = (await browser.runtime.sendMessage({
          type: 'RUC_LIBRARY_RESERVATION_RECORDS_QUERY',
          params,
        })) as LibraryReservationRecordsQueryResult | undefined

        if (currentRequest !== requestVersion) return

        if (!result || typeof result.ok !== 'boolean') {
          error.value = '图书馆预约记录返回了无效结果。'
        ...
        if (!result.ok) {
          error.value = result.error || '图书馆预约记录查询失败。'
