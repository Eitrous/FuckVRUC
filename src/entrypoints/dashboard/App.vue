...
<template>
  <main class="min-h-screen bg-neutral-950 text-neutral-100">
    ...
    <section class="mx-auto max-w-6xl px-6 py-10">
      <header class="mb-8">
        <h2 class="mt-2 text-3xl font-semibold">课表</h2>
      </header>
      <button @click="querySchedules('2025-2026-2')">查询课表</button>
      <p v-if="schedule_error" class="mt-4 text-sm text-red-400">
        {{ schedule_error }}
      </p>

      <p v-if="schedule_fetchedAt" class="mt-4 text-xs text-neutral-500">
        更新时间：{{ new Date(schedule_fetchedAt).toLocaleString() }}
      </p>

      <div v-if="schedules.length" class="mt-5 overflow-x-auto">
        <table class="w-full text-left text-sm">
          <thead class="border-b border-neutral-800 text-neutral-400">
            <tr>
              <th class="py-2">课程</th>
              <th class="py-2">学分</th>
              <th class="py-2">上课时间</th>
              <th class="py-2">上课地点</th>
              <th class="py-2">教师</th>
            </tr>
          </thead>
